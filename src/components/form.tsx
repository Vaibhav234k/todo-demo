...
export function TodoForm() {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setTodolist([...todoList,{ id: todoId++, item: input }])
    console.log(input);
  };
      ...
      <form className="my-8" onSubmit={handleSubmit}>
        <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4">
          <LabelInputContainer className="grow">
            <Input id="firstname" onChange={(e)=>{setInput(e.target.value)}} placeholder="ex. Go to gym today" type="text" />
          </LabelInputContainer>
          <button
          ...
          <button onClick={(id) => {
             const editedTodo = prompt("Edit the todo:");
              if (editedTodo !== null && editedTodo.trim() !== '') { 
                setTodolist(
                  todoList.map(todo =>
                    todo.id === id ? { ...todo, item: editedTodo } : todo
                  )
                ); 
